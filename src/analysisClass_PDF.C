...
void analysisClass::Loop()
{
   ...
   TH1F *h_NeventsRatio = new TH1F ("NeventsRatio","NeventsRatio",400,0.8,1.2);
   TH1F *h_NeventsRatioPassed = new TH1F ("NeventsRatioPassed","NeventsRatioPassed",400,0.8,1.2);
   TH1F *h_EffVariation = new TH1F ("EffVariation","EffVariation",2000,-0.2,0.2);

   /////////initialize variables
   float Ngen=0;
   float Npassed=0;
   float NeventsWeighted[41];
   float NeventsWeightedPassed[41];
   for (int i=0; i<41; i++) NeventsWeightedPassed[i]=0;
   for (int i=0; i<41; i++) NeventsWeighted[i]=0;
   ...
   //for (Long64_t jentry=0; jentry<100;jentry++) {
     ...
     Ngen++;
     for (int i=0; i<41; i++) {
      //cout << PDFweight[i] << endl;
	   NeventsWeighted[i]=NeventsWeighted[i]+PDFweight[i]; 
      }
     ...
     if( v_idx_ele.size() >= 1 ) 
       {
	 fillVariableWithValue( "Pt1stEle", elePt[v_idx_ele[0]] );
       }
     if( v_idx_ele_ID.size() >= 1 ) 
       {
	 fillVariableWithValue( "Pt1stEleID", elePt[v_idx_ele_ID[0]] );
       }
     if( v_idx_ele_ISO.size() >= 1 ) 
       {
	 fillVariableWithValue( "Pt1stEleISO", elePt[v_idx_ele_ISO[0]] );
       }
     ...
     if( v_idx_ele.size() >= 2 ) 
       {
	 fillVariableWithValue( "Pt2ndEle", elePt[v_idx_ele[1]] );
       }
     if( v_idx_ele_ID.size() >= 2 ) 
       {
	 fillVariableWithValue( "Pt2ndEleID", elePt[v_idx_ele_ID[1]] );
       }
     if( v_idx_ele_ISO.size() >= 2 ) 
       {
	 fillVariableWithValue( "Pt2ndEleISO", elePt[v_idx_ele_ISO[1]] );
       }
     ...
     if( passedCut("all") )
       {
	 Npassed++;  //keep track of the total number of events that pass
	 for (int i=0; i<41; i++) {
	   //cout << PDFweight[i] << endl;
	   NeventsWeightedPassed[i]=NeventsWeightedPassed[i]+PDFweight[i]; //if this event passes, add wieght to total
	 }
	 if (diff_11_22<diff_12_21) 
	   {
	     h_Mej->Fill(M11);
   ...
   for (int i=1; i<41; i++){ 
     //cout << NeventsWeightedPassed[i]/Npassed << endl;
     h_NeventsRatioPassed->Fill(NeventsWeightedPassed[i]/Npassed);
     float Eff = Npassed/Ngen; //calculate unweight signal efficiency
     float EffVar = (Eff - (NeventsWeightedPassed[i]/NeventsWeighted[i]))/Eff; //calculate change in efficiency with PDF reweights
     //cout << Eff << "\t" << EffVar << endl;
     h_EffVariation->Fill(EffVar);
   }
   for (int j=1; j<41; j++){
     h_NeventsRatio->Fill(NeventsWeighted[j]/Ngen);
   }
   h_NeventsRatio->Write();
   h_NeventsRatioPassed->Write();
   h_EffVariation->Write();
