...
void analysisClass::Loop()
{
   ...
   //for (Long64_t jentry=0; jentry<50000;jentry++) {
     ...
     float deltaR_minCut_MuEle = getPreCutValue1("muon_ele_DeltaRcut");
	 ...
	 /////remove muon tracks close to electron tracks
	 //Disambiguation of electrons from muons
	 float minDeltaRMuEle=9999.0;
	 TVector3 ele1_vec;
	 ele1_vec.SetPtEtaPhi(elePt[iele],eleEta[iele],elePhi[iele]);
	 for (int i=0; i <muonCount ; i++){
	   TVector3 muon1_vec;
	   muon1_vec.SetPtEtaPhi(muonPt[i],muonEta[i],muonPhi[i]);
	   double distanceEleMu = muon1_vec.DeltaR(ele1_vec);
	   if (distanceEleMu<minDeltaRMuEle) minDeltaRMuEle=distanceEleMu;
	 }
	 
	 if (minDeltaRMuEle>deltaR_minCut_MuEle ) pass_ele = true;
	 

	 if ( (pass_ID) && (pass_ISO) && (pass_ele) ) v_idx_ele_final.push_back(iele);
	 ...
	 if ( minDeltaR > deltaR_minCut_2 ) pass_DIS = true;
     ...
     //## nMuon
     fillVariableWithValue( "nMu_noCut", v_idx_muon_noCut.size() ) ;
     fillVariableWithValue( "nMu_PtPreCut", v_idx_muon.size() ) ;
     fillVariableWithValue( "nMu_PtPreCut_ID", v_idx_muon_ID.size() ) ;
     fillVariableWithValue( "nMu_PtPreCut_ISO", v_idx_muon_ISO.size() ) ;
     fillVariableWithValue( "nMu_PtPreCut_DIS", v_idx_muon_DIS.size() ) ;
     fillVariableWithValue( "nMu_PtPreCut_IDISO_DIS", v_idx_muon_final.size() ) ;
     
     
     //## nMu
     if(v_idx_muon_noCut.size()>=2) fillVariableWithValue("nMuon_noCut", v_idx_muon_noCut.size());

     if(v_idx_muon.size()>=2) fillVariableWithValue("nMuon_PtPreCut", v_idx_muon.size());

     if(v_idx_muon_ID.size()>=2) fillVariableWithValue("nMuon_PtPreCut_ID", v_idx_muon_ID.size());

     if(v_idx_muon_ISO.size()>=2) fillVariableWithValue("nMuon_PtPreCut_ISO", v_idx_muon_ISO.size());

     if(v_idx_muon_DIS.size()>=2) fillVariableWithValue("nMuon_PtPreCut_DIS", v_idx_muon_DIS.size());
     
     if(v_idx_muon_final.size()>=2) fillVariableWithValue("nMuon_PtPreCut_IDISO_DIS", v_idx_muon_final.size());
     
     //cout << "1st and 2nd mu" << endl;
     
     //## 1st and 2nd mu
     if(v_idx_muon.size()>=2) {
       if( muonPt[v_idx_muon[0]] > muonPt[v_idx_muon[1]] ){
	 fillVariableWithValue( "Pt1stMu", muonPt[v_idx_muon[0]] );
	 fillVariableWithValue( "Pt2ndMu", muonPt[v_idx_muon[1]] );
       }
       else{
	 fillVariableWithValue( "Pt1stMu", muonPt[v_idx_muon[1]] );
	 fillVariableWithValue( "Pt2ndMu", muonPt[v_idx_muon[0]] );
       }
     }
     if(v_idx_muon_ID.size()>=2) {
       if( muonPt[v_idx_muon_ID[0]] > muonPt[v_idx_muon_ID[1]] )
	 {
	   fillVariableWithValue( "Pt1stMuID", muonPt[v_idx_muon_ID[0]] );
	   fillVariableWithValue( "Pt2ndMuID", muonPt[v_idx_muon_ID[1]] );
	 }
       else{
	 fillVariableWithValue( "Pt1stMuID", muonPt[v_idx_muon_ID[1]] );
	 fillVariableWithValue( "Pt2ndMuID", muonPt[v_idx_muon_ID[0]] );
       }
     }
     if(v_idx_muon_ISO.size()>=2) {
       if( muonPt[v_idx_muon_ISO[0]] > muonPt[v_idx_muon_ISO[1]] ){
	 fillVariableWithValue( "Pt1stMuISO", muonPt[v_idx_muon_ISO[0]] );
	 fillVariableWithValue( "Pt2ndMuISO", muonPt[v_idx_muon_ISO[1]] );
       }
       else{
	 fillVariableWithValue( "Pt1stMuISO", muonPt[v_idx_muon_ISO[1]] );
	 fillVariableWithValue( "Pt2ndMuISO", muonPt[v_idx_muon_ISO[0]] );
       }
     }
     if(v_idx_muon_DIS.size()>=2) {
       if( muonPt[v_idx_muon_DIS[0]] > muonPt[v_idx_muon_DIS[1]] ){
	 fillVariableWithValue( "Pt1stMuDIS", muonPt[v_idx_muon_DIS[0]] );
	 fillVariableWithValue( "Pt2ndMuDIS", muonPt[v_idx_muon_DIS[1]] );
       }
       else{
	 fillVariableWithValue( "Pt1stMuDIS", muonPt[v_idx_muon_DIS[1]] );
	 fillVariableWithValue( "Pt2ndMuDIS", muonPt[v_idx_muon_DIS[0]] );
       }
     }
     if(v_idx_muon_final.size()>=2){
       if( muonPt[v_idx_muon_final[0]] > muonPt[v_idx_muon_final[1]] ){
	 fillVariableWithValue( "Pt1stMuIDISO_DIS", muonPt[v_idx_muon_final[0]] );
	 fillVariableWithValue( "Pt2ndMuIDISO_DIS", muonPt[v_idx_muon_final[1]] );
	 fillVariableWithValue( "Eta1stMuIDISO_DIS", muonEta[v_idx_muon_final[1]] );
	 fillVariableWithValue( "Eta2ndMuIDISO_DIS", muonEta[v_idx_muon_final[0]] );
       }
       else{
	 fillVariableWithValue( "Pt1stMuIDISO_DIS", muonPt[v_idx_muon_final[1]] );
	 fillVariableWithValue( "Pt2ndMuIDISO_DIS", muonPt[v_idx_muon_final[0]] );
	 fillVariableWithValue( "Eta1stMuIDISO_DIS", muonEta[v_idx_muon_final[1]] );
	 fillVariableWithValue( "Eta2ndMuIDISO_DIS", muonEta[v_idx_muon_final[0]] );
       }
     ...
     //## define "2muons" and "2jets" booleans
     bool TwoMuons=false;
     bool TwoJets=false;

     if(v_idx_muon_final.size() >= 2 ) TwoMuons = true;
     ...
     if( TwoMuons ) 
       {
	 TLorentzVector v_mumu, muon1, muon2;
	 muon1.SetPtEtaPhiM(muonPt[v_idx_muon_final[0]],
			  muonEta[v_idx_muon_final[0]],
			  muonPhi[v_idx_muon_final[0]],0);
	 muon2.SetPtEtaPhiM(muonPt[v_idx_muon_final[1]],
			   muonEta[v_idx_muon_final[1]],
			   muonPhi[v_idx_muon_final[1]],0);
	 v_mumu = muon1 + muon2;
	 fillVariableWithValue( "invMass_mm", v_mumu.M() ) ;
       }
     ...
     if ( (TwoMuons) && (TwoJets) ) 
       {
 	 TLorentzVector jet1, jet2, muon1, muon2;
	 muon1.SetPtEtaPhiM(muonPt[v_idx_muon_final[0]],
			   muonEta[v_idx_muon_final[0]],
			   muonPhi[v_idx_muon_final[0]],0);
	 muon2.SetPtEtaPhiM(muonPt[v_idx_muon_final[1]],
	 ...
	 TLorentzVector jet1muon1, jet1muon2, jet2muon1, jet2muon2;
	 jet1muon1 = jet1 + muon1;
	 jet1muon2 = jet1 + muon2;
	 jet2muon2 = jet2 + muon2;
	 jet2muon1 = jet2 + muon1;
	 M11 = jet1muon1.M();
	 M12 = jet1muon2.M();
	 M21 = jet2muon1.M();
	 M22 = jet2muon2.M();
     ...
     if(passedCut("all")){
       float weight=0.0350398;//ttbar
       //float weight=0.00910217;//LQ250
       //float weight=0.00029306;//LQ400
       //float weight=0.0115939;//VV+jets
       //float weight=0.038286;//Z+jets


       if (diff_11_22<diff_12_21) {
	 h_Mlj->Fill(M11,weight);
	 h_Mlj->Fill(M22,weight);
       }
       else {
	 h_Mlj->Fill(M12,weight);
	 h_Mlj->Fill(M21,weight);
       }
     }
