...
void analysisClass::Loop()
{
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
	   ...
	   && abs(GenParticlePdgId[GenParticleMotherIndex[igen]])==MotherPID)
	  {
		...
		if (InBarrel)
		  if ((eleNumTrkIso[idx_minDeltaR]<eleNumTrkIso_barrel_cut)&&(eleTrkIso[idx_minDeltaR]<eleTrkIso_barrel_cut)
		      &&(eleReducedEcalIso[idx_minDeltaR]<(eleEcalRecHitIso_barrel_cut+(eleEcalRecHitIso_barrel_cut2*elePt[idx_minDeltaR])))
		      &&(eleHcalTowerIso[idx_minDeltaR]<(eleHcalRecHitIso_barrel_cut+(eleHcalRecHitIso_barrel_cut2*elePt[idx_minDeltaR])))
		      )
		    PassISO=true;
		
		if (InEndCap)
		  if ((eleNumTrkIso[idx_minDeltaR]<eleNumTrkIso_endcap_cut)&&(eleTrkIso[idx_minDeltaR]<eleTrkIso_endcap_cut)
		      &&(eleReducedEcalIso[idx_minDeltaR]<(eleEcalRecHitIso_endcap_cut+(eleEcalRecHitIso_endcap_cut2*elePt[idx_minDeltaR])))
		      &&(eleHcalTowerIso[idx_minDeltaR]<(eleHcalRecHitIso_endcap_cut+(eleHcalRecHitIso_endcap_cut2*elePt[idx_minDeltaR])))
		      )
		    PassISO=true;
		...
		if (InBarrel){
		  ...
		  if( eleReducedEcalIso[idx_minDeltaR]<(eleEcalRecHitIso_barrel_cut+(eleEcalRecHitIso_barrel_cut2*elePt[idx_minDeltaR])) )
		    passISO_EcalIso=true;

		  if( eleHcalTowerIso[idx_minDeltaR]<(eleHcalRecHitIso_endcap_cut+(eleHcalRecHitIso_endcap_cut2*elePt[idx_minDeltaR])) )
		...
		if (InEndCap){
		  ...
		  if( eleReducedEcalIso[idx_minDeltaR]<(eleEcalRecHitIso_endcap_cut+(eleEcalRecHitIso_endcap_cut2*elePt[idx_minDeltaR])) )
		    passISO_EcalIso=true;
	...
	if ((eleNumTrkIso[iele]<eleNumTrkIso_barrel_cut)&&(eleTrkIso[iele]<eleTrkIso_barrel_cut)
	    &&(eleReducedEcalIso[iele]<(eleEcalRecHitIso_barrel_cut+(eleEcalRecHitIso_barrel_cut2*elePt[iele])))
	    &&(eleHcalTowerIso[iele]<(eleHcalRecHitIso_barrel_cut+(eleHcalRecHitIso_barrel_cut2*elePt[iele])))
	    )
	  PassISO=true;
      
      if (InEndCap)
	if ((eleNumTrkIso[iele]<eleNumTrkIso_endcap_cut)&&(eleTrkIso[iele]<eleTrkIso_endcap_cut)
	    &&(eleReducedEcalIso[iele]<(eleEcalRecHitIso_endcap_cut+(eleEcalRecHitIso_endcap_cut2*elePt[iele])))
	    &&(eleHcalTowerIso[iele]<(eleHcalRecHitIso_endcap_cut+(eleHcalRecHitIso_endcap_cut2*elePt[iele])))
	    )
	  PassISO=true;
