...
void analysisClass::Loop()
{
   ...
   TH1F *h_ele_E_SingEle = new TH1F ("ele_E_SingEle","ele_E_SingEle",100,0,1000);
   TH1F *h_ele_Pt_SingEle  = new TH1F ("ele_Pt_SingEle","ele_Pt_SingEle",100,0,1000);
   TH1F *h_ele_Phi_SingEle  = new TH1F ("ele_Phi_SingEle","ele_Phi_SingEle",71,-3.55,3.55);
   TH1F *h_ele_Eta_SingEle  = new TH1F ("ele_Eta_SingEle","ele_Eta_SingEle",121,-6.05,6.05);
   TH1F *h_ele_CaloEnergy_SingEle  = new TH1F ("ele_CaloEnergy_SingEle","ele_CaloEnergy_SingEle",100,0,1000);


   /////////initialize variables
   double EventsPassed_80=0;
   double EventsPassed_200=0;
   double EventsPassed_SingEle=0;
   double n_Events=0;
   ...
   for (Long64_t jentry=0; jentry<1;jentry++) {
     ...
     string Trig3="HLT_IsoEle18_L1R";  //??This doesn't seem to be like the SingleElec
	 ...
	 bool IsSingEle = !strncmp(Trig3.c_str(),tmp.c_str(),12);
	 //if ((IsEM80)||(IsEM200)) cout << "Trigger:  " << tmp << "  HLTResults " << HLTResults[results_index] << endl;
	 if (IsSingEle) cout << "Trigger:  " << tmp << "  HLTResults " << HLTResults[results_index] << endl;
	 ...
	 if ((IsSingEle)&&(HLTResults[results_index]==true)) {
	   EventsPassed_SingEle++;
	   for (int iele=0; iele<eleCount; iele++) {
	     if (elePt[iele]<elePt_cut) continue;
	     h_ele_E_SingEle->Fill(eleEnergy[iele]);
	     h_ele_Pt_SingEle->Fill(elePt[iele]);
	     h_ele_Phi_SingEle->Fill(elePhi[iele]);
	     h_ele_Eta_SingEle->Fill(eleEta[iele]);
	     h_ele_CaloEnergy_SingEle->Fill(eleCaloEnergy[iele]);
	   } //for iele	   
	 }
   ...
   h_ele_E_SingEle->Write();
   h_ele_Pt_SingEle->Write();
   h_ele_Phi_SingEle->Write();
   h_ele_Eta_SingEle->Write();
   h_ele_CaloEnergy_SingEle->Write();
   ...
   cout << "Events Passed Trigger SingEle: " << EventsPassed_SingEle << "  Fraction: " << EventsPassed_SingEle/n_Events <<endl;
